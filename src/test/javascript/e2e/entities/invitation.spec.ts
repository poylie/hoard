...
describe('Invitation e2e test', () => {
    ...
    it('should load Invitations', () => {
        entityMenu.click();
        element.all(by.css('[routerLink="invitation"]')).first().click().then(() => {
            const expectVal = /Invitations/;
    ...
    it('should load create Invitation dialog', () => {
        element(by.css('button.create-invitation')).click().then(() => {
            const expectVal = /Create or edit a Invitation/;
