...
describe('Group e2e test', () => {
    ...
    it('should load Groups', () => {
        entityMenu.click();
        element.all(by.css('[routerLink="group"]')).first().click().then(() => {
            const expectVal = /Groups/;
    ...
    it('should load create Group dialog', () => {
        element(by.css('button.create-group')).click().then(() => {
            const expectVal = /Create or edit a Group/;
